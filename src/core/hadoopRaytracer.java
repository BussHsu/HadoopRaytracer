...
public class hadoopRaytracer {
	...
	private static int mode =0; // mode = 0 (default), 1(alternate)

	public static class Map extends MapReduceBase
			implements Mapper<PairWritable, BytesWritable, IntWritable, BytesWritable> {
		public void map(PairWritable keyrow, BytesWritable scenebytes,
			...
			for (int line = start; line <= end; line++) {
				...
				for (int i = 0; i < xRes; i++) {
					System.arraycopy(raytracer.getColor3f(scene, i, rowid).getBytes(), 0, rgbs,
							i * 3 + (line - start) * 3 * xRes, 3);
				}
			}
	...
	public static class Reduce extends MapReduceBase
			implements Reducer<IntWritable, BytesWritable, NullWritable, BytesWritable> {

		public void reduce(IntWritable keyrow, Iterator<BytesWritable> values,
				OutputCollector<NullWritable, BytesWritable> output, Reporter reporter) throws IOException {
	...
	public static int byte2int(byte[] res, int shift) {
		...
		int targets = (res[0 + shift] & 0xff) | ((res[1 + shift] << 8) & 0xff00) | ((res[2 + shift] << 24) >>> 8)
				| (res[3 + shift] << 24);
		return targets;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (args[2].equals("-m")) {
			...
			try {divide = Integer.parseInt(args[1]);}
			catch(Exception e) {divide = 0;}
		...
		} else if (args[0].equals("-d") || args[0].equals("--divide")) {
			try {
